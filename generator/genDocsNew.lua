...
local uiLocation = "docs\\api\\ui\\"

-- UI classes are written to the ui/ subfolder so the autogenerated sidebar groups them under a
-- "UI" category. Each still gets a `slug: /api/<Name>` so URLs (and the emmy "View Documentation"
-- links) stay /api/<Name> regardless of the folder.
local uiClasses = {
    Widget = true, Button = true, ButtonPanel = true, Checkbox = true, Colorbox = true,
    Graph = true, Header = true, Inputbox = true, Label = true, Panel = true, Selectbox = true,
    Separator = true, Slider = true, Vectorbox = true, Window = true, UI = true,
}
...
function genDocs:generateClassFile(name, class, nameOrig)
    ...
    local isUI = uiClasses[nameOrig]
    local filename = (isUI and uiLocation or docsLocation)..name..".mdx"
    if not util:file_exists(filename) then
        local fileW = io.open(filename, "w") --add the file if it was missing
        genDocs:addFrontMatter(name, fileW, isUI and ("/api/"..util:firstToUpper(name)) or nil)
        fileW:close()
    end
    ...
    --write intro. re-emit the `Client`/`Server` badges right AFTER the frontmatter's closing '---'
    --(keying off a fixed line number breaks when frontmatter has an extra field like `slug:`)
    local fmEnd, dashes = 0, 0
    for i, line in ipairs(intro) do
        if line == "---" then dashes = dashes + 1; if dashes == 2 then fmEnd = i break end end
    end
    for i, line in ipairs(intro) do
        --dont write existing client/server, they're re-added below
        if not (string.find(line, "`Client`") or string.find(line, "`Server`")) then
            file:write(line, "\n")
        end
        if i == fmEnd then
            if (onClient) then file:write("`Client`", "\n") end
            if (onServer) then file:write("`Server`", "\n") end
        end
    end
...
function genDocs:addFrontMatter(filename, fileWrite, slug)
    ...
    if slug then frontMatter[#frontMatter + 1] = "slug: "..slug end
