...
function self:checkCollision()
    ...
    local hit = c:Raycast(self.obj.pos, vel)[1]
    if hit then
...
function self:updateEdit()
    ...
    if self.lastPos ~= self.obj.pos then
        if self.lastPos then
            --deb:line(self.lastPos, self.obj.pos + vel * 1.5)
            --self:eraseLine(self.lastPos, self.obj.pos + vel * 1.5)
            self:makeLine(self.lastPos, self.obj.pos)
        end
        self.lastPos = self.obj.pos
    end
end
...
function self:makeLine(p1, p2)
    ...
    local thickness = 0.1 * self.obj.scale
...
function self:eraseLine(p1, p2)
    --multiply thickness by transform scale
    local thickness = self.obj.scale * 0.5
    local s = Capsule(p1, p2, thickness)
