...
### VoxelEdit VoxelEdit() {#VoxelEdit-VoxelEdit}

### VoxelEdit VoxelEdit(Shape shape) {#VoxelEdit-VoxelEdit-Shape-shape}

### VoxelEdit VoxelEdit(Shape shape, Vec3 position) {#VoxelEdit-VoxelEdit-Shape-shape-Vec3-position}
...
### Image CaptureImage(Mat4 textureUvTmWorld, number relBorderSize) {#Image-CaptureImage-Mat4-textureUvTmWorld-number-relBorderSize}

### Image GetImageResource(string name) {#Image-GetImageResource-string-name}

### nil FreeTmpLayers(integer clientId) {#nil-FreeTmpLayers-integer-clientId}
...
### nil FillTmpLayers(TmpLayerFlags layerFlags, Vec3 camPos, Vec3[] targets, number targetRadius, integer clientId) {#nil-FillTmpLayers-TmpLayerFlags-layerFlags-Vec3-camPos-Vec3-targets-number-targetRadius-integer-clientId}
It projects cone on voxel geometry and everything inside will be copies to tmp
all raycasts will collide with this tmp layer instead until FreeTmpLayers
parameters specify "cone" - two positions and end radius

### string[] GetMaterialNames() {#string-GetMaterialNames}

### table GetMaterial(string materialName) {#table-GetMaterial-string-materialName}

### nil SetMaterial(string materialName, table matData) {#nil-SetMaterial-string-materialName-table-matData}

### nil SetStaticSceneMaterial(string materialName, number roughness, number metallicity) {#nil-SetStaticSceneMaterial-string-materialName-number-roughness-number-metallicity}

### nil SetVoxelDataResourceMaterial(VoxelData vd, string materialName) {#nil-SetVoxelDataResourceMaterial-VoxelData-vd-string-materialName}

### nil ReplaceVoxelDataResourceMaterial(VoxelData vd, string destMaterialName, string srcMaterialName, boolean clearSourceMat) {#nil-ReplaceVoxelDataResourceMaterial-VoxelData-vd-string-destMaterialName-string-srcMaterialName-boolean-clearSourceMat}
...
### Object copySourceTr {#Object-copySourceTr}

### Object copyDestinationTr {#Object-copyDestinationTr}
...
### VoxelData copyResource {#VoxelData-copyResource}

### boolean colorEnable {#boolean-colorEnable}

### boolean normalEnable {#boolean-normalEnable}

### boolean roughnessEnable {#boolean-roughnessEnable}

### boolean metallicityEnable {#boolean-metallicityEnable}

### boolean materialEnable {#boolean-materialEnable}
...
### table&lt;string,number&gt; removeList {#tablestringnumber-removeList}

### table&lt;string,number&gt; carveList {#tablestringnumber-carveList}

### string[] removeCountList {#string-removeCountList}
...
### boolean imageTriplanar {#boolean-imageTriplanar}
Triplanar projection — samples the image along world X/Y/Z so it covers every face, with the tiling grid anchored in world space. When false, the image is planar-projected along a single direction.

### boolean imageNormalOverwrite {#boolean-imageNormalOverwrite}
When true, the image's normal map replaces the surface normal (flattening to bare geometry if the texture has no normal); when false it blends into the existing surface normal.

### boolean imageProjectionFade {#boolean-imageProjectionFade}
Planar projection only: fade the albedo on faces angled away from the projection axis (decal-style, lands only on the facing face). Ignored for triplanar, which covers every face.

### number imageNormalBlendPower {#number-imageNormalBlendPower}

### fun(progress:number) onProgress {#fun-progressnumber-onProgress}
callback function. progress from 0-1. May not be called every frame. Is called after script updates 

### fun(info:RemoveCountInfo, computeTimeMs:number) onFinished {#fun-infoRemoveCountInfo-computeTimeMsnumber-onFinished}
